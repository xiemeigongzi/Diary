...
2。 在 main() 函数里， 创建一个 StaticAccessNonStatic 的对象，
	用这个对象调用 info() 方法， 胡总嗯好用类去调用这个 info() 方法
...
public class PersonTest
{
	public static void main(String[] args)
	{
		System.out.println("Person 的Field 值： "+Person.eyeNum);
		
		Person p= new Person();
		System.out.println("p 变量的 name field 值是： "+p.name 
		+" p 变量的 eyeNum field 值： "+ p.eyeNum);
		...
		System.out.println("p2 变量的 eyeNum field 值： "+ p2.eyeNum);
	}
}
